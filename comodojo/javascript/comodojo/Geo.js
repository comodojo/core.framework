	...
	startup_location: function() {
		...
		if (this.lat !== false && this.lng !== false) {
			...
			this.locate().then(function(position) {
				...
				$c.Error.generic(false,$c.getLocalizedMessage('10044'),error);
			});
			this.zoom();
	...
	geocodeCallback: function(results, status) {
		...
		if (status == "OK") {
			...
			if (results.length != 1) {
				...
				$c.Dialog.select($c.getLocalizedMessage('10045'),'',content,this.geocodeResolve);
			}
			else {
	...
	reverseGeocodeCallback: function(results, status) {
		...
		if (status == "OK") {
			...
			if (results.length != 1) {
				...
				$c.Dialog.select($c.getLocalizedMessage('10045'),'',content,this.reverseGeocodeResolv);
			}
			else {
