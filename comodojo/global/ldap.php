...
/********************* PUBLIC METHODS ********************/
	 ...
	 */
	public function __construct($server, $port=636, $dcs, $dns, $filter, $listuser=null, $listpass=null, $cmode=true) {
...
/********************* PRIVATE METHODS *******************/	
	...
	private function setupConnection() {
		...
		if (!$this->ldaph) {
			comodojo_debug('LDAP server '.$this->server.' is not responding','ERROR','ldap');
			return false;
		}
	...
	private function listDirectoryHelper() {
		...
		$result = ldap_search($this->ldaph,trim($this->dcs).($this->dns !== NULL ? ",".trim($this->dns) : ''),$this->listFilter);
		if (!$result) {
